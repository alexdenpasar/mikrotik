...
# file name for system backup - file name will be Mikrotik-servername-date-time-no-encryption
:local fname1 ("/Mikrotik-".[/system identity get name]."-".$ds."-".$ts."-no-encryption")
...
/tool fetch address="$ftphost" port=$ftpport src-path=(/.$fname1.".rsc") dst-path=("$ds".(/.$fname1.".rsc")) user="$ftpuser" mode=ftp password="$ftppassword" upload=yes
:delay 10s;
