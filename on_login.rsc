...
# exception words. Here we add our "legitimate" IPs, or, for example, services on which we do not want track connections.
:local removeThese {"name1";"name2";"192.168.0.1"}
# smtp server
:local smtpserv "smtp.server.com"
# email send
:local email "email@email.com"
# user email
:local useremail "mikrotik"
# password email
:local pass "password"
:local sysname [/system identity get name];
# END SETUP
#we check whether the entry in the scheduler exists, if not, we return an error in the log
:if ([:len [/system scheduler find name="$scheduleName"]] = 0) do={
  /log warning "[LOGMON] ERROR: Schedule does not exist. Create schedule and edit script to match name"
}
#determine the last time the script was run
...
if ([:len $output] > 0) do={
  ...
  /tool e-mail send server=$smtpserv port=587 user=$useremail password=$pass start-tls=yes to=$emailAddress  from=$email subject="MikroTik $sysname alert $currentTime" body="$output"
